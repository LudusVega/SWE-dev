...
def _upgrade_legacy_structure(data):
    ...
    if isinstance(sample_value, dict):
        ...
        if isinstance(nested_candidate, dict) and 'complete' in nested_candidate:
        ...
        if 'complete' in sample_value or 'completed_by' in sample_value:
            legacy = status
            upgraded = {'legacy': {}}
            for sheet_number, payload in legacy.items():
                upgraded['legacy'][sheet_number] = {
                    'complete': bool(payload.get('complete', False)),
                    'narrative_complete': bool(payload.get('narrative', False)),
